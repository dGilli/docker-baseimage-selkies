...
These are future-plan items requested by the user, tracked here so they survive compaction. Each is an open **GitHub issue** on `dGilli/docker-baseimage-selkies`, kept on the project board — canonical mapping + commands: `toc.md#GH-Tracking`.

1. **Implement GPU use for desktop rendering** → **GH #4**
...
2. **Improve project CLI and UX** → **GH #5**
...
3. **Fix selkies menu app installer** → **GH #6** (follow-on catalog = **#10**)
...
4. **Implement proper project and fork maintenance workflow** → **GH #7**
...
5. **Implement proper SLU image registry** → **GH #8**
...
Also tracked as open issues (outside the 6-item list above): **GH #11** aarch64 RHEL9 variant · **GH #12** Wayland (phase-2).
