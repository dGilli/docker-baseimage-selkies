...
- Commits: one logical change per commit. **Subject style = upstream house style** (user preference, 2026-09-01, F74):
  ...
  - **Headings**: NEVER reference the Memory Bank in a commit heading — no MB paths, no MB-internal identifiers (e.g. build labels like `c9`); an MB reference belongs in the body, and only when needed (user rule, 2026-09-01; the fixed journal heading above is the one sanctioned MB heading).
...
- **Delta budget**: modified-upstream files are capped at the allowlist in `scripts/delta-allowlist.txt` (currently 6: the 5 distro-guarded shared-tree files + `Dockerfile` whole-file ownership). Adding a 7th requires editing the allowlist in the SAME PR with justification (the allowlist's git history is the delta audit). RHEL-specific content is ALWAYS additive (`root-base/`, `deploy/`, `memory-bank/`, `scripts/`, new `root/` files). `package_versions.txt` is branch-owned generated content per upstream convention, not a separate additive file.
- **Generated files** — take the baseline verbatim, never hand-edit: `Jenkinsfile`, `README.md`, `package_versions.txt`, `jenkins-vars.yml`, upstream `.github/workflows/*`. (`Dockerfile` is branch-OWNED — the main image, allowlist [modified]; `Dockerfile.aarch64` is intentionally absent until RHEL9 arm64 work, allowlist [intentionally-absent]. `.github/workflows/fork-maintenance.yml` is OURS — additive.)
