...
## Task: SLU-specific README — branch-owned (started 2026-09-21)

**State**: `PLAN v2 (awaiting approval)`. User directive: remove the "README.md is generated by the upstream builder — never hand-edit" rule; the fork builds locally (podman), does not run LS Jenkins. Reclassify `README.md` → `[modified]` whole-file ownership (mirrors the `Dockerfile` precedent, `scripts/delta-allowlist.txt:19-24`), then write the SLU RHEL9 README = user's NOTE + fleshed-out RHEL9 sections + verbatim upstream **master** README body under `BEGIN/END UNMODIFIED UPSTREAM README` markers. **v2 (user, 2026-09-21)**: `deploy/` is being moved to a separate SLU "provisioning" project — this repo = image only. README must NOT reference `deploy/`; Quick start carries a self-contained kubectl recipe (manifest = the template's rendered production defaults: 2 CPU/4Gi, port 3000, svc 8080→3000, haproxy ingress `<name>.nrp-nautilus.io`, dnsPolicy None 8.8.8.8/8.8.4.4, SYS_PTRACE, emptyDir /config+/dev/shm, `selkies-password` secret, PSA must not be restricted/baseline; GPU addendum = nvidia.com/gpu:1 + DISABLE_ZINK + SELKIES_AUTO_GPU + Recreate, M3 = XSERVER_BACKEND=nvidia-xorg, 2–4 min first boot).
**Plan facts (all verified 2026-09-21)**: current delta = **8 modified + 113 additive** (`projectRules.md:35` "6" is stale — M3 added `svc-xorg/run` + `svc-selkies/run`); `LICENSE` = GPLv3; f44 README = 3-line pointer, full doc lives on **master** (fetched `@ 0bd500a`, 2026-09-21); NRP contract = single port 3000 + same-origin ws (`selkies-rhel9.yaml.template:23-31`); deploy GPU env = `SELKIES_AUTO_GPU=true` (`--gpu`, `apply-nrp-e2e.sh:238`), `XSERVER_BACKEND=nvidia-xorg` (`--gpu-xorg`, `:246`). Rule locations to edit: `delta-allowlist.txt:11,65`; `projectRules.md:4,35-36`; `systemPatterns.md:40,47`; `build-deployment.md:58`; `findings.md` F71 (→ superseded by new F80). No CI change: `upstream-delta.sh` reclassifies from the allowlist automatically.
**Decision items for user**: D1 embed **master** (not f44) README body [assumed yes — f44's is a pointer]; D2 drop master's 2 boilerplate HTML comment lines from the embedded body [recommended]; D3 keep upstream's trailing "loop testing" artifact line [recommended keep — auditable "unmodified"].
**QA plan**: `upstream-delta.sh` PASS (9 modified, incl. `ok(M): README.md`) + fidelity diff of the embedded section vs a fresh master fetch = 0.
**2026-09-22 (user-directed, NO COMMIT)**: BUILD'd a short `## Development` section near the top of the working-tree README (between intro bullets and `## Quick start`): credits the [opencode](https://opencode.ai) agent (repo: github.com/sst/opencode) as a co-developer and explains `memory-bank/` is the agent's persistent journal (session context/task state/findings/decisions) — internal record-keeping, not project docs, no effect on built images. Allowlist reclassification still pending (README.md still `[generated]` at `scripts/delta-allowlist.txt:78`); this edit rides on the same in-flight README work. NOTE 2026-09-21 (session): the `## Development` section is absent from BOTH the working-tree README and the target repo README (diff = trailing whitespace only); superseded by the SLU-migration README decision below (target verbatim wins).

## Task: SLU org migration + sanitized history (approved 2026-09-21)

**State**: `BUILD` (user approved PLAN v4 + amendments: OCI labels added to Dockerfile before registry push; issues/projects stay on dGilli fork; SLU repo branch `rhel9` replaces `main`; CI trigger = `rhel9`).

**Target**: `slu-end-user-services/docker-baseimage-selkies` (standalone repo, NOT a GitHub fork; initial 3 commits by user, tree = README + LICENSE).

**History design (10 commits, root = `upstream/fedora44` `0d5ee26a76fe8a48bab19f08aa13e1c0bab35fdf` = tag `fedora44-de2e97f2-ls43`)**:
1. `add README.md with project information` — user's `8fe0039` re-rooted (author/date/message preserved; diff vs f44 = README.md 3-line pointer → fork README)
2. `Revise README with updates and new sections` — user's `95f4d71` re-rooted
3. `add SLU-owned RHEL UBI9 base + Xvfb/openbox/selkies stack` — from `9dc267e`; writes `Dockerfile` DIRECTLY (no `Dockerfile.rhel9` ever), deletes `Dockerfile.aarch64`; **OCI `org.opencontainers.image.*` labels added here** (title/description/source/url/vendor/licenses + existing build_version/maintainer)
4. `add standard rhel gnome-shell 40.10 as default X11 desktop` — from `f50af76`
5. `refactor proot-apps 0.3.2 + RHEL9 bwrap stub` — from `7e67680`
6. `keep memory-bank out of version control (local journal)` — M `.gitignore` (+`memory-bank/`)
7. `add upstream delta allowlist, delta report, and fork CI workflow tooling` — from `3caafdf`, authored final-correct: CI trigger `branches: [rhel9]`; allowlist [modified] from start = Dockerfile, startwm.sh, init-nginx/run, init-selkies-config/run, svc-docker/run, selkies-proot, .gitignore, README.md; [intentionally-absent] = Dockerfile.aarch64; [generated] complete (4 f44 workflows + Jenkinsfile + package_versions.txt + jenkins-vars.yml)
8. `restore svc-dbus system bus for rhel9 to fix GNOME 40 power indicator crashes` — from `7aeca4d` (F67)
9. `add GPU desktop rendering via Xorg + NVIDIA DDX + NVENC` — `650939f` image-split (+allowlist svc-xorg/run, svc-selkies/run)
10. `fix proot-apps on nodes with YAMA ptrace_scope=2` — `e38aa7b` image-split (Dockerfile + init-selkies-config/run only)

**Dropped**: user's `d87a02d` license commit (GPLv3 byte-identical to f44 baseline `LICENSE`, from upstream `54876a3` — would be empty) · `deploy/` 3 files (`c9ef304`, `fca920c`, `f7d01df` parts — separate SLU provisioning project) · all 9 MB commits (MB detached: local gitignored journal; full NRP-inclusive MB history preserved on frozen dGilli `rhel9`) · f44-bot generated-file hunks in `650939f`/`e38aa7b` (free with f44-tip root) · pin-bump `7f1d82f` · local working-tree README variant (trailing-ws-trimmed → breaks master@0bd500a verbatim embed; target's verbatim copy wins).

**Scrubs (folded into introducing commits)**: 13 NRP comment lines — `Dockerfile:16,47,180,183` (→ commit 3), `startwm.sh:12,21` (→ 4), `init-selkies-config/run:283,292` (→ 10), `selu-xorg-config:7,32` (→ 9), `perf-baseline.md:3,22` + `perf-regression.sh:3` (→ 9); commit bodies of 3/4/9/10 scrubbed. Final tracked tree = zero NRP refs except `nautilus`/`nautilus-extensions` package names (`package_versions_rhel9.txt:624,635`, `Dockerfile:249`).

**Delta projection**: 10 modified (8 image/shared + `.gitignore` + `README.md`) + 88 additive + 0 drift + `Dockerfile.aarch64` intentionally absent → gate PASS.

**Push procedure**: build branch locally (worktree) → plain `git push slu <branch>:rhel9` (new branch, no force) → `gh repo edit --default-branch rhel9` → delete old `main` → branch protection on `rhel9`.

**Local worktree migration**: `origin` → SLU repo · dGilli remote → `dgilli-ref` · local frozen branch `rhel9` → renamed `pre-sanitize` · checkout new `rhel9` (tracks origin) · discard local README.md edit · `git restore --source=pre-sanitize --worktree -- memory-bank/` (untracked + gitignored journal resumes).

**Image**: build from new tip on this entitled host → `ghcr.io/slu-end-user-services/docker-baseimage-selkies:rhel9` + `:rhel9-v1` pin. GHCR auth: no stored `auth.json` on host → `podman login ghcr.io` with `gh auth token` (token scopes `repo` ⇒ GHCR push OK for dGilli-writable repos).

**Rollback anchors**: dGilli frozen `rhel9` (→ `pre-sanitize`) + `rhel9-dev` @ `7015647` + registry `dgilli/selkies-rhel9:v6-llvmpipe` = `10c646b392e2` + old SLU `main` @ `d87a02d` (deletable — all content re-rooted).
