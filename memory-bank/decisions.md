...
**Consequences**: GNOME + Settings + Firefox were live-validated on a GPU node with `DISABLE_ZINK=true` and dynamic resolution. M2 remains open until that behavior is codified and re-validated through the normal committed deploy path, not live `kubectl set env` only.
**References**: F59–F63; `activeContext.md#GPU-M2-live-debug-state`; `root/defaults/startwm.sh:4-8`; `Dockerfile.rhel9:190-195`
...
**Consequences**: `deploy/nrp/selkies-rhel9.yaml.template` and `apply-nrp-e2e.sh --gpu` should render without manual resolution env; clean M2 rerun must verify browser-driven resizing.
**References**: F63; `activeContext.md#GPU-M2-live-debug-state`; `svc-xorg/run:23-34`; `svc-de/run:28-40`; selkies 348bc4f `settings.py`/`selkies.py`
